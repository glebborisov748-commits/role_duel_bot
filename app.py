...
PRODUCTS = {
    ...
    # ELITE — первый товар с РЕАЛЬНОЙ конвертацией вместо "рубли=звёзды 1:1": 1 звезда стоит
    # реально ~$0.014 (официальный курс Telegram, TON/200 — не курс покупки через App
    # Store/Google Play, там дороже из-за комиссии самих Apple/Google, ~$0.02), при курсе ЦБ
    # ~84.26₽/$ (12.09.2026) это ~1.18₽/звезда. 1200₽ по этому курсу ≈ 1017⭐, округлено до
    # 1000⭐ — то есть звёзды честно дешевле рублей по номиналу, а не "рубли=звёзды" как раньше.
...
# Цена в рублях (для Lava), если не задана явно (как теперь у ELITE) — по-прежнему равна
# цене в звёздах один в один. setdefault, а не прямое присваивание, — чтобы не затереть
# ELITE-исключение выше.
