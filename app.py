...
def get_user(user_id):
    ...
    if user_id not in user_data:
        user_data[user_id] = {
            ...
            # Стартовый бонус: 276/12 = 23 сообщения до первого "засыпания" вместо обычных ~12.5
            # (ENERGY_MAX=150) — чтобы новый пользователь не упёрся в стену почти сразу.
            # ENERGY_MAX как потолок регена/энергетиков этим не затрагивается.
            "energy": 276,
            "satiety": 100,
            "energizers": 0,
            "bucks": 0,
...
ENERGIZER_RESTORE_AMOUNT = 98  # 65% от нового бака в 150 (150*0.65=97.5, округлено). Пробовали
                               # уполовинить и вдвое увеличить все количества (тот же суммарный
                               # эффект, цифры выглядят щедрее) — по фидбэку "72 энергетика за 200"
                               # воспринималось как абсурдно много само по себе, откатили обратно.
...
BUNDLES = {
    ...
    "bundle_large": {"energizers": 50, "bucks": 500, "emoji": "👑",
                      # 200⭐/50 = 4.0⭐ за энергетик — чуть дешевле, чем у PRO (250⭐/60=4.17⭐):
                      # бандл отдаёт всё сразу одним куском, а подписка размазывает по 30 дням,
                      # так что небольшая премия за мгновенность оправдана. Раньше было 36 (5.56⭐/
                      # энергетик — хуже PRO); пробовали поднять до 72 психологическим трюком
                      # (уполовинить силу энергетика) — само число выглядело абсурдным, откатили.
